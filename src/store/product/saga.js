...
function* get({ id }) {
    try {
        ...
        yield put(LIST_SUCCESS_ACTION(response))
    } catch (error) {
...
function* fetch({conditional, limit, offset}) {
    try {
        ...
        yield put(GET_SUCCESS_ACTION(response.data, response.meta));
    } catch (error) {
        yield put(GET_FAILED_ACTION(error))
    }
}

function* register({ payload: { product, history } }) {
    try {
        const response = yield call(POST_API_REQUEST, product)
...
function* update({ payload: { id, product, history } }) {
    try {
        const response = yield call(PUT_API_REQUEST, id, product)
